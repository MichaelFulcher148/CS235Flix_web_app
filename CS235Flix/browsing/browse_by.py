...
def browse_by_title():
    ...
    return render_template('list_movies.html', left_arrow=left_link, right_arrow=right_link, initials=first_letters, first_initial=first_letter_pick, movie_list=services.get_movies_by_title(first_letter_pick, repo.repository_instance))

@browse_blueprint.route('/browse_by_genre')
def browse_by_genre():
    genre_list = repo.repository_instance.get_genres()
    ...
    if genre_pick is None:
        ...
        if services.get_pop_of_genre(genre_pick, repo.repository_instance) > 20:
            ...
            return render_template('browse_by_genre.html', genre=genre_pick, genre_list=genre_list, left_arrow=left_link, right_arrow=right_link, initials=first_letters, movie_list=movie_list)
        return render_template('browse_by_genre.html', genre=genre_pick, genre_list=genre_list, movie_list=services.get_movies_by_genre(genre_pick, repo.repository_instance))
...
def browse_by_director():
    ...
    if director_pick is None:
        return render_template('browse_by_director.html', left_arrow=left_link, right_arrow=right_link, initials=first_initials, first_initial=first_initial_pick, director_list=services.get_directors(first_initial_pick, repo.repository_instance))
    else:
        return render_template('browse_by_director.html', left_arrow=left_link, right_arrow=right_link, initials=first_initials, first_initial=first_initial_pick, director_list=services.get_directors(first_initial_pick, repo.repository_instance), movie_list=services.get_movies_by_director(director_pick, repo.repository_instance), director_name=director_pick)
...
def browse_by_actor():
    ...
    if actor_pick is None:
        return render_template('browse_by_actor.html', left_arrow=left_link, right_arrow=right_link, initials=first_initials, first_initial=first_initial_pick, actor_list=services.get_actors(first_initial_pick, repo.repository_instance))
    else:
        return render_template('browse_by_actor.html', left_arrow=left_link, right_arrow=right_link, initials=first_initials, first_initial=first_initial_pick, actor_list=services.get_actors(first_initial_pick, repo.repository_instance), movie_list=services.get_movies_by_actor(actor_pick, repo.repository_instance), actor_name=actor_pick)
